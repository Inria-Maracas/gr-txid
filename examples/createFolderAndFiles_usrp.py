...
for rx_idx, rx in enumerate(rx_nodes):
	...
	for node_id in range(1, NUM_NODES):
		if node_id == rx: # Write the command to launch RX on this node
			...
			yaml_content += f"      command: reciever.py -R re_{rx_idx:02d} -I im_{rx_idx:02d} -T {len(tx_nodes)}\n\n"  # TODO Add num samples to save?
			continue
		...
		if node_id == args.sched_node: # TODO Allow scheduler to run on a Tx or Rx node?
			...
			yaml_content +=  "    container:\n"
			yaml_content += f"    - image: {docker_img_string}\n"
			yaml_content += f"      command: scheduler.py -p {args.port_num} -s {args.tx_freq} -n {tx_node_string}\n"
		...
		if node_id in tx_nodes:
			...
			yaml_content +=  "    container:\n"
			yaml_content += f"    - image: {docker_img_string}\n"
			yaml_content += f"      command: emitter.py -P {args.port_num} -T {tx_nodes.index(node_id)} -G {default_gain} -R {int(args.is_random)} -r {int(args.is_noise)} -f {pow_var_speed * int(args.is_var)} -M 0.5\n"
			yaml_content +=  "    passive: true\n\n"
