...
namespace gr {
  namespace txid {
    ...
    correlator::sptr
    correlator::make(const std::vector<gr_complex> &symbols, float sps, unsigned int mark_delay, float threshold, tm_type threshold_method)
    {
      return gnuradio::make_block_sptr<correlator_impl>(
     ...
     */
    correlator_impl::correlator_impl(const std::vector<gr_complex> &symbols, float sps, unsigned int mark_delay, float threshold, tm_type threshold_method)
    ...
    std::vector<gr_complex> correlator_impl::symbols() const { return d_symbols; }

    void correlator_impl::set_symbols(const std::vector<gr_complex>& symbols)
    {
    ...
    unsigned int correlator_impl::mark_delay() const { return d_mark_delay; }

    void correlator_impl::_set_mark_delay(unsigned int mark_delay)
    {
    ...
    void correlator_impl::set_mark_delay(unsigned int mark_delay)
    {
        gr::thread::scoped_lock lock(d_setlock);
        _set_mark_delay(mark_delay);
    }

    float correlator_impl::threshold() const { return d_thresh; }

    void correlator_impl::_set_threshold(float threshold)
    {
    ...
    void correlator_impl::set_threshold(float threshold)
    {
        gr::thread::scoped_lock lock(d_setlock);
        ...
        gr_vector_void_star &output_items)
    {
      ...
      // Get the input signal power
      volk_32fc_magnitude_squared_32f(&d_mag[0], in, noutput_items+hist_len);
