...
from gnuradio import gr, blocks, fft, analog, digital, txid
...
class ofdm_rx(gr.hier_block2):
    ...
    |           entirely. Also used for coarse frequency offset and
    |           channel estimation.
    check_len:  Check if recieved frame length is as expected. Set to expected value, or -1 to not check
    """
    ...
    def __init__(self, fft_len=_def_fft_len, cp_len=_def_cp_len,
        ...
        if scramble_bits:
            ...
            self.scramble_seed = 0x00  # We deactivate the scrambler by init'ing it with zeros
        ### Sync ############################################################
        sync_detect = digital.ofdm_sync_sc_cfb(fft_len, cp_len, threshold=self.sc_thresh)
        ...
        header_demod = digital.constellation_decoder_cb(
            header_constellation.base())
        header_formatter = txid.packet_header_ofdm(
            occupied_carriers, 1,
